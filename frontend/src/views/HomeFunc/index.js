...
function App() {
  ...
  function handleDeleteItemFromCart(item) {
    const nowItems = [...cartItems];
    const delItem = { ...item };
    delItem.inCart = false;
    const baru = nowItems.filter((it) => it.id !== delItem.id);
    setCartItems(baru);
    updateShopItem(delItem, false);
    setBalance(balance + delItem.price);
  }
        ...
        <div className="row mt-3">
          {!cartHidden ? (
            <div className="col-sm">
              <List title="My Cart" items={cartItems} onItemClick={handleDeleteItemFromCart}></List>
            </div>
          ) : (
            <div className="col-sm">
              <List title="List Items" items={shopItems} onItemClick={handleAddItemToCart} isShopList={true}></List>
            </div>
          )}
        </div>
      </div>
