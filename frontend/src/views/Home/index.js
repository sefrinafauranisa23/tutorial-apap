...
export default class Home extends React.Component {
  ...
  handleDeleteItemFromCart = (item) => {
    const nowItems = [...this.state.cartItems];
    const delItem = { ...item };
    delItem.inCart = false;
    const baru = nowItems.filter((it) => it.id !== delItem.id);
    this.setState({ cartItems: baru });
    this.updateShopItem(delItem, false);
    this.setState({ balance: this.state.balance + delItem.price });
  };
          ...
          <div className="row mt-3">
            {!this.state.cartHidden ? (
              <div className="col-sm">
                <List title="My Cart" items={this.state.cartItems} onItemClick={this.handleDeleteItemFromCart}></List>
              </div>
            ) : (
              <div className="col-sm">
                <List title="List Items" items={this.state.shopItems} onItemClick={this.handleAddItemToCart} isShopList={true}></List>
              </div>
            )}
          </div>
        </div>
